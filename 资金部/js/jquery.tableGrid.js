		...
		this.each(function(){
			...
			$(thisTable).children("tbody").on('click','tr',function(){
				...
				if(classFlag){
					...
					//复选框也可以变成单选
					if($(checkbox).hasClass("radioSelect")){
			...
			if(options.paging){
				...
				// 点击go跳转
				$(thisPage).on('click','#gotoPage',function(){
					...
					var totalPage_number=$("#_totalPage").val();
					if(pageNumber>=parseInt(totalPage_number)){
						alert("输入页数不能超过"+totalPage_number);return;
					}
					$("#_click_page").val(pageNumber);
					options.pageAjax();
				});
				
				$(thisPage).on('keydown','#page_number',function(){
					 if(event.keyCode == "13") {
							var pageNumber=$("#page_number").val();
							if(pageNumber.length==0){
								alert("请输入要跳转的页码！"); return;
